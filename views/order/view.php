...
$this->registerCssFile('@web/css/order-view.css', ['depends' => app\assets\AppAsset::className()]);
$totalNumber = $totalPrice = 0;
?>
<div id="main">
    <div class="Detail_inf">
        <div class="Detail_inf_shang">
            <?php if ($model->status == \app\models\Order::PAYMENT_STATUS_UNPAID || $model->status == \app\models\Order::PAYMENT_METHOD_COD) { ?><a id="order-cancel" href="javascript:void(0);" style="float:right; margin-top:18px;" data-link="<?= Yii::$app->urlManager->createUrl(['order/ajax-status', 'id' => $model->id, 'status' => \app\models\Order::STATUS_CANCEL]) ?>">取消订单</a><?php } ?>
            <ul>
                <li>订单号：<?= $model->sn ?></li>
                <li>状态： <span> <?= \app\models\Order::getStatusLabels($model->status) ?> </span> </li>
                <li>
    ...
    <div class="Order_information">
        ...
        <div class="goods-list">
            ...
            <ul>
                <?php 
                	/*echo "<pre>";
                	print_r($model->orderProducts);
                	echo "</pre>";*/
                foreach ($model->orderProducts as $product) { ?>
                <li class="cle">
                    <div class="pic"> <a target="_blank" href="<?= Yii::$app->urlManager->createUrl(['product/view', 'id' => $product->product_id]) ?>"> <img src="#" alt="<?= $product->name ?>"> </a> </div>
